defmodule FormMetal.Filters.NaiveDatetime.OnOrAfter do
  @moduledoc """
  The `on_or_after` operator for the `naive_datetime` value.
  """
  ...
  defimpl FormMetal.Filters.Testers.InMemory do
    ...
    def test(filter) do
      nil_guard(filter) do
        NaiveDateTime.compare(filter.source, filter.value) in [:eq, :gt]
      end
    end
  end
end
